...
Alien::Alien(GameObject& associated, int nMinions, float offset) : Component(associated){
...
void Alien::Start(){
    for(int i = 0; i < nMinions; i++){
        ...
        Minion* minion = new Minion(*mgo, Game::GetInstance().GetCurrentState().GetObjectPtr(&associated), ((i*M_PI)/3));
        mgo->AddComponent(minion);
        minionArray.push_back(Game::GetInstance().GetCurrentState().AddObject(mgo));
    }
}
